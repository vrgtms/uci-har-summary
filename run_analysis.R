...
## (4) Change column names to descriptive labels
colnames(merged) <- gsub("[\\(\\)-]", "", colnames(merged)) #Remove special chars

# Expand abbreviations
colnames(merged) <- gsub("Freq", "Frequency", colnames(merged))
colnames(merged) <- gsub("^t", "TimeDomain", colnames(merged))
colnames(merged) <- gsub("^f", "FrequencyDomain", colnames(merged))
...
colnames(merged) <- gsub("std", "StandardDeviation", colnames(merged))
